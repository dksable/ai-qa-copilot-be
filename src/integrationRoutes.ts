...
function onboardingFileContent(filePath: string, testFolderPath = "tests") {
  ...
  if (filePath === ".github/workflows/playwright-validation.yml") {
    return [
      ...
      "        with:",
      "          path: node_modules",
      "          key: ${{ runner.os }}-node-modules-${{ hashFiles('package-lock.json', 'npm-shrinkwrap.json') }}",
      "          restore-keys: |",
      ...
      "          key: ${{ runner.os }}-playwright-${{ github.event.inputs.browser }}-${{ hashFiles('package-lock.json', 'npm-shrinkwrap.json') }}",
      "          restore-keys: |",
      "            ${{ runner.os }}-playwright-${{ github.event.inputs.browser }}-",
      "      - name: Install dependencies",
      "        shell: bash",
      "        run: |",
      "          if [ -d node_modules/@playwright/test ]; then",
      "            echo 'Dependencies already restored from cache. Skipping npm install.'",
